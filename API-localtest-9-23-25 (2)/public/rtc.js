...
// Part 1: Globally declared variables and conditions
var pc = [];
var socketId = '';
...
var hasTherapistUsrName = false;
var timerHasBeenSet = false;
var reconnecting = false;
var isSharingScreen = false;
var isBlurringLocalBkground = false;
var isMirroringLocalVideo = false;
var flipFlopBorderColor = false; // every time a video is spawned, we will flip this and can use it for any alternating design
...
var usernamesList = []; // used for nametags, sent from server with socketID match, eventually
var videoList = []; // save local objects containing video elem and id
const room = ROOM_ID;
var hasSetupUserNameUniq = false;
var username = '';
var user = '';
var stringdisplayname = ''; // actual data returned with no formatting
var TherapistUser = '';
var hasSessionEnded = false;
var hasStream = false;
var videoSavedStartWidth = {};
var videoSavedStartHeight = {};
var dragableSelectedVideoID = -1;
var hasCustomResetUsername = false;

// Lots of functions that were disordered but don't need to be redefined are in this block (line39to2553~)
// Start function clump
    async function DismissCurrentClient(id, socketid) {
        if(socketid == socket.io.engine.id) { return; }
        socketid = socketid.split('-video');
        socketid = socketid[0];
    ...
    const setChatSettingsBtn = document.querySelector(".main__chat_window");
    ...
    const CreateChatMessage = (data) => {
        ...
        `<div class="message">
            <b><i class="far fa-user-circle"></i> <span> ${
            data.userName === username && !hasCustomResetUsername ? userNameTemp.slice(0,-2) : userNameTemp
            }</span> </b>
            <span>${data.message}</span>
        </div>`;
    };
    ...
    backBtn.addEventListener("click", () => {
        if(window.outerWidth<1080) // mobile
        {
            ...
            let changeNameElem = document.getElementById('changeChatSettingsModal');
            if(changeNameElem) changeNameElem.style.setProperty('display', 'none');
        }
        else
        {
            document.querySelector(".main__right").style.display = "none";
    ...
    if(setChatSettingsBtn)
    {
        setChatSettingsBtn.addEventListener("click", () => {
            //let localusername = prompt("Enter your name");
            //if(localusername) { username = localusername; user = localusername; }
            let changeNameElem = document.getElementById('changeChatSettingsModal');
            if(changeNameElem) changeNameElem.style.setProperty('display', 'block');
        });

        const setChatUserSubmitBtn = document.querySelector("#changeChatSettingsButton");
        const setChatUserCloseBtn = document.querySelector("#changeChatSettingsCloseButton");
        setChatUserSubmitBtn.onclick = function()
        {
            let inputChatNameElem = document.querySelector("#inputChatName");
            let inputChatNameDescriptorElem = document.querySelector("#inputChatNameDescriptor");
            if(inputChatNameElem)
            {
                let currentInput = inputChatNameElem.value;

                // Check length and values
                if(!currentInput || currentInput.length==0)
                {
                    if(inputChatNameDescriptorElem) inputChatNameDescriptorElem.innerHTML = '<span style="color:red;">Nothing entered.</span>';
                    return;
                }
                else if(currentInput.length<=1)
                {
                    if(inputChatNameDescriptorElem) inputChatNameDescriptorElem.innerHTML = '<span style="color:red;">Length must be 2 or larger.</span>';
                    return;
                }

                // Set value
                user = username = currentInput;
                hasCustomResetUsername = true;

                // Set success
                if(inputChatNameDescriptorElem) inputChatNameDescriptorElem.innerHTML = '<span style="color:green;">Successfully set new name.</span>';

                // Close box in 3 seconds
                setTimeout( ()=>{
                    let changeNameElem = document.getElementById('changeChatSettingsModal');
                    if(changeNameElem) changeNameElem.style.setProperty('display', 'none'); 
                }, 3000);
        ...
        setChatUserCloseBtn.onclick = function()
        {
            let changeNameElem = document.getElementById('changeChatSettingsModal');
            if(changeNameElem) changeNameElem.style.setProperty('display', 'none');
        }
    ...
    // Stop propogation; clicking anywhere but messages opens the Chat Settings
    if(messages)
    {
    ...
    async function requestClientStatusRecursive()
    {
        ...
        response.json().then(data => {
            ...
            if(TherapistUser) 
            {
                ...
                cpanelButton.style.display = 'flex';
    ...
    function ChangeRoomLook()
    {
        ...
        {
            document.documentElement.style.setProperty("--main-dark", "rgb(235,235,255)");
            document.documentElement.style.setProperty("--main-darklg", "rgb(22, 33, 200)");
    ...
    // Change video size by scale value
    function changeVideoScaleX(currentSelectedVideo, newAmount)
    {
        //console.log("changevideoscalex", currentSelectedVideo, newAmount);
        if(!currentSelectedVideo) return;
        let currentVideoId = currentSelectedVideo.id;
        let newWidth = Math.ceil(videoSavedStartWidth[currentVideoId] * newAmount);
        currentSelectedVideo.style.setProperty("width", '' + newWidth + 'px');

        if(newAmount>1) currentSelectedVideo.style.setProperty("max-width", '' + newWidth + 'px');
        else currentSelectedVideo.style.setProperty("max-width", '35vw');

        if(newAmount == 1) currentSelectedVideo.style.setProperty('flex', '1 0 22%');
        else currentSelectedVideo.style.removeProperty('flex');
    }
    // Change video size by scale value
    function changeVideoScaleY(currentSelectedVideo, newAmount)
    {
        //console.log("changevideoscaley", currentSelectedVideo, newAmount);
        if(!currentSelectedVideo) return;
        let currentVideoId = currentSelectedVideo.id;
        let newHeight = Math.ceil(videoSavedStartHeight[currentVideoId] * newAmount);
        currentSelectedVideo.style.setProperty("height", '' + newHeight + 'px');

        if(newAmount == 1) currentSelectedVideo.style.removeProperty('height');

        /*if(currentSelectedVideo == myVideo)
        {
            if(newAmount>1) myVideo.style.setProperty("max-height", '' + newHeight + 'px');
            else myVideo.style.setProperty("max-height", '188px');
        }*/
    }

    // Add dragability events to element
    function addDragability(selectedElement)
    {
        //let container = document.getElementById('video-grid');
        let container = document.getElementsByClassName('main__left')[0];
        let isDragging = false;
        let startX, startY;
        selectedElement.addEventListener('mousedown', (e) => {
            if(selectedElement.id != dragableSelectedVideoID) return;
            isDragging = true;
        ...
        document.addEventListener('mousemove', (e) => {
            ...
            // Keep video inside the container
            newX = Math.max(0, Math.min(newX, container.offsetWidth-12 - selectedElement.offsetWidth));
            newY = Math.max(0, Math.min(newY, container.offsetHeight+24 - selectedElement.offsetHeight));

            //selectedElement.style['margin-left'] = newX + 'px';
            //selectedElement.style['margin-top'] = newY + 'px';

            selectedElement.style['left'] = newX + 'px';
            selectedElement.style['top'] = newY + 'px';
            selectedElement.style['margin-top'] = '0px';
            selectedElement.style['margin-left'] = '0px';
        });
    ...
    // Reset z-indexes based on selected video or local video
    function maintainVideoZIndexes()
    {
        ...
        for(let i=0;i<videoElems.length;i++)
        {
            ...
            if(videoElem.id != 'local') videoElem.style.setProperty('z-index', 'auto');
            else videoElem.style.setProperty('z-index', 2);
        }
    }
    ...
    function OpenOtherStreamingOptions(e)
    {
        ...
        // Reset sizing or button/scales to default or whatever it was
        if(currentSelectedVideo != e.target)
        {
            optionsScaleVideoButtonX.value = 1.0;
            optionsScaleVideoButtonY.value = 1.0;
            optionsDragVideoButton.checked = false;
        ...
        // Set this video to have highest z-index for now
        maintainVideoZIndexes();
        currentSelectedVideo.style.setProperty('z-index', 3);

        // Get video size at start
        let style = currentSelectedVideo.style;
        let width = '400px';
        if(style.cssText.includes("width")) { width = currentSelectedVideo.style.width; }
        else { width = '' + currentSelectedVideo.offsetWidth + 'px'; currentSelectedVideo.style.setProperty("width", currentSelectedVideo.offsetWidth); }
        let widthnumber = width.replace(/\D/g, "");
        if(widthnumber) widthnumber = parseInt(widthnumber);
        if(videoSavedStartWidth[videoID] === undefined) videoSavedStartWidth[videoID] = widthnumber;
        let height = '400px';
        if(style.cssText.includes("height")) { height = currentSelectedVideo.style.height; }
        else { height = '' + currentSelectedVideo.offsetWidth + 'px'; currentSelectedVideo.style.setProperty("height", currentSelectedVideo.offsetHeight); }
        let heightnumber = height.replace(/\D/g, "");
        if(heightnumber) heightnumber = parseInt(widthnumber);
        if(videoSavedStartHeight[videoID] === undefined) videoSavedStartHeight[videoID] = heightnumber;
    ...
    const optionsScaleVideoButtonX = document.querySelector("#streamingOptionsModal2scaling");
    const optionsScaleVideoButtonY = document.querySelector("#streamingOptionsModal2scalingy");
    const optionsDragVideoButton = document.querySelector("#streamingOptionsModal2draggable");
    const optionsOtherResetMovementsButton = document.querySelector("#optionsButtonResetMovements");
    const myoptionsOtherVolumeButton = document.querySelector("#mystreamingOptionsModal2volume");
    const myoptionsScaleVideoButtonX = document.querySelector("#mystreamingOptionsModal2scaling");
    const myoptionsScaleVideoButtonY = document.querySelector("#mystreamingOptionsModal2scalingy");
    const myoptionsDragVideoButton = document.querySelector("#mystreamingOptionsModal2draggable");
    const myoptionsOtherResetMovementsButton = document.querySelector("#myoptionsButtonResetMovements");
    ...
    {
        changeVideoVolume(currentSelectedVideo, optionsOtherVolumeButton.value);
    }
    optionsScaleVideoButtonX.onchange = function()
    {
        changeVideoScaleX(currentSelectedVideo, optionsScaleVideoButtonX.value);
    }
    optionsScaleVideoButtonY.onchange = function()
    {
        changeVideoScaleY(currentSelectedVideo, optionsScaleVideoButtonY.value);
    }
    optionsDragVideoButton.onclick = function()
    {
        if(!currentSelectedVideo) return;
        if(optionsDragVideoButton.checked) { dragableSelectedVideoID = currentSelectedVideo.id; currentSelectedVideo.style.setProperty('position', 'absolute'); }
        else dragableSelectedVideoID = -1;
        //console.log(dragableSelectedVideoID, optionsDragVideoButton.checked);
    }
    optionsOtherResetMovementsButton.onclick = function()
    {
        if(!currentSelectedVideo) return;
        let currentVideoId = currentSelectedVideo.id;
        if(videoSavedStartWidth[currentVideoId] !== undefined) { currentSelectedVideo.style.setProperty('width',''+videoSavedStartWidth+'px'); currentSelectedVideo.style.setProperty('max-width','35vw'); }
        if(videoSavedStartHeight[currentVideoId] !== undefined) currentSelectedVideo.style.removeProperty('height'); 
        if(dragableSelectedVideoID == currentVideoId) 
        {
            dragableSelectedVideoID = -1;
            optionsDragVideoButton.checked = false;
        }
        currentSelectedVideo.style.removeProperty('margin-left');
        currentSelectedVideo.style.removeProperty('margin-top');
        currentSelectedVideo.style.removeProperty('position');
        optionsScaleVideoButtonX.value = 1;
        optionsScaleVideoButtonY.value = 1;
    }
    myoptionsScaleVideoButtonX.onchange = function()
    {
        changeVideoScaleX(myVideo, myoptionsScaleVideoButtonX.value);
    }
    myoptionsScaleVideoButtonY.onchange = function()
    {
        changeVideoScaleY(myVideo, myoptionsScaleVideoButtonY.value);
    }
    myoptionsDragVideoButton.onclick = function()
    {
        if(!myVideo) return;
        if(myoptionsDragVideoButton.checked) { dragableSelectedVideoID = myVideo.id; myVideo.style.setProperty('position', 'absolute'); }
        else dragableSelectedVideoID = -1;
        //console.log(dragableSelectedVideoID, myoptionsDragVideoButton.checked);
    }
    myoptionsDragVideoButton.checked = false;
    myoptionsOtherResetMovementsButton.onclick = function()
    {
        if(!myVideo) return;
        let currentVideoId = myVideo.id;
        if(videoSavedStartWidth[currentVideoId] !== undefined) { myVideo.style.removeProperty('width'); myVideo.style.setProperty('max-width','244px'); }
        if(videoSavedStartHeight[currentVideoId] !== undefined) { myVideo.style.removeProperty('height'); myVideo.style.setProperty('max-height','188px'); }
        if(dragableSelectedVideoID == currentVideoId) 
        {
            myVideo.style.setProperty('margin-left', '8rem');
            myVideo.style.setProperty('margin-top', '1rem');
            dragableSelectedVideoID = -1;
            myoptionsDragVideoButton.checked = false;
        }
        myVideo.style.removeProperty('position');
        myoptionsScaleVideoButtonX.value = 1;
        myoptionsScaleVideoButtonY.value = 1;
    }
    ...
    function removeAllVideoTrackElements()
    {
    ...
    {
        if(socket) socket.emit("settimer", { room: room, timer: 0 });
    }
    ...
    async function optionsHidePanels()
    {
        ...
        let getCPanelISearch = document.querySelector("#modalSearchFrame");
        if(getCPanelISearch) getCPanelISearch.style.display = 'none';
...
// Part 0 shared or required Functions
var roomStatus = 0;
...
{
    let elem = document.getElementById('noStreamDetectedModal');
    if(elem) elem.display = 'block';
    await until(_ => myStream);
...
const disconnectedNotificationBtn = document.querySelector("#disconnectedNotificationModalButton");
disconnectedNotificationBtn.onclick = function()
{
    let elem = document.getElementById('disconnectedNotificationModal');
...
// Startup function, includes first parts of the loading process that do not need to be re-done
function Startup()
{
        // Setup my video element
        if(!myVideo || !document.getElementById('local'))
        {
            myVideo = document.createElement("video");
            myVideo.muted = true;
            let videoId = 'local';
            myVideo.id = videoId;
            myVideo.style.setProperty('z-index', 2);
            addDragability(myVideo);

            // Get video size at start or shortly afterward (size may not be appended yet, if not does not matter much)
            setTimeout( ()=> {
                let style = myVideo.style;
                let width = '400px';
                width = '' + myVideo.offsetWidth + 'px';
                let widthnumber = width.replace(/\D/g, "");
                if(widthnumber) widthnumber = parseInt(widthnumber);
                if(videoSavedStartWidth[videoId] === undefined) videoSavedStartWidth[videoId] = widthnumber;
                let height = '400px';
                height = '' + myVideo.offsetHeight + 'px';
                let heightnumber = height.replace(/\D/g, "");
                if(heightnumber) heightnumber = parseInt(widthnumber);
                if(videoSavedStartHeight[videoId] === undefined) videoSavedStartHeight[videoId] = heightnumber;
            }, 1500);
...
async function setupUserVideoPart2()
{
    try
    {
    await navigator.mediaDevices.getUserMedia({audio: {echoCancellation: true,noiseSuppression: true},video: false}).then((stream) => {
        myStream = stream;
        addVideoStream(myVideo, stream);
        helpers.setLocalStream( myVideo, stream );
        hasStream = true;
        attemptedErrorSetup = true;
    });
    } catch (err) {
        hasStream = false;
        ...
        myStream = new MediaStream([black(24,24), silence()]);

        if(myStream)
        {
            addVideoStream(myVideo, myStream);
            helpers.setLocalStream( myVideo, myStream);
            hasStream = true;
        }
    }
}

async function setupUserVideo()
{
    try
    {
...
// Setup connection stream/sockets
function setupConnectionSockets()
{
    socket = io( '/stream' ); // have to intialize this connection here, otherwise it's a race condition where 'connect' is not defined fast enough
    socket.on( 'connect', () => {
        //set socketId
        ...
        socket.on( 'ice candidates', async ( data ) => {
			...
			/*if (data.candidate !== undefined && data.candidate) {
            let candidateType = data.candidate.type;
            let candidateProtocol = data.candidate.protocol; // e.g., 'udp', 'tcp'

            console.log(`ICE Candidate Type: ${candidateType}`);
            console.log(`ICE Candidate Protocol: ${candidateProtocol}`);

            if (candidateType === 'srflx') {
                console.log('STUN server is being used for this candidate.');
            } else if (candidateType === 'relay') {
                console.log('TURN server is being used for this candidate.');
            } else if (candidateType === 'host') {
                console.log('Direct host connection for this candidate.');
            }
			}*/
        } );
        ...
        socket.on( 'disconnectedDismissed', (data) => {
            ...
            setTimeout( ()=> { removeAllVideoTrackElements(); alert("You have been disconnected from this session. If you believe this is an error, then you may try to reconnect.") }, 500);
            setTimeout( ()=> { removeAllVideoTrackElements(); }, 3000);
        });
            ...
            if ( data.description.type === 'offer' ) {
                ...
                else*/
                {
                    ...
                    navigator.mediaDevices.getUserMedia({audio: {echoCancellation: true,noiseSuppression: true},video: true}).then( async (stream) => {
                         ...
                         socket.emit( 'sdp', { description: pc[data.sender].localDescription, to: data.sender, sender: socketId } );
                         //console.log('sdp', { description: pc[data.sender].localDescription, to: data.sender, sender: socketId });
                     } ).catch( async ( e ) => {
                        //alert("No microphone or video stream was detected on your machine.")
                        console.log( e );

                        // Create a blank video track
                        //let blankVideoTrack = new MediaStreamTrack();

                        //const stream = new MediaStream();
                        //pc[data.sender].addTrack(blankVideoTrack, stream);
...
function RTCinit( createOffer, partnerName ) {
    ...
    pc[partnerName].ontrack = ( e ) => {
        ...
        if (elem) { elem.srcObject = str; addDragability(elem); }
        else 
        {
            //video elem
...
window.addEventListener( 'load', async () => {
    ...
    // Start video and local variables
    Startup();
    ...
    // Setup recursive client status polling for waiting room and other options
     ...
     let localClientStatusRet = await requestClientStatus().then(data => {
         roomStatus = data.status;
         if(roomStatus == -1) return EndSession(); // If it isn't a valid room then we won't have a session
         ArrangeSidebarHandler();
         CheckSessionEnded();
         ChangeRoomLook();
     });
    ...
    // Login or setup user name
    TherapistUser = sessionStorage.getItem( 'BMUserChecksum' ) || localStorage.getItem( 'BMUserChecksum' ) || getCookie('BMUserChecksum');
    TherapistUser = '540609175'; // for test
    ...
    // Handle/display user startup message
    ...
    //Get user video or audio by default, set hasMediaToUse if either function fails, so we won't try to get our media streams later
    setupUserVideo();

    // Make notification if stream was setup, then connect; otherwise make fake stream and connect
    /*if(!hasStream)
    {
        myStream = new MediaStream();
        if(myStream)
    ...
    await checkForStream();
    hasStream = true;

    // Setup connection
    setupConnectionSockets();
} );
